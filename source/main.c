...

{
	...
	char fileNameInput[150] = "/mnt/d/github/visual_try_on/PatchMatch/image_files/forest/forest.bmp";
	char fileNameMasked[150] = "/mnt/d/github/visual_try_on/PatchMatch/image_files/forest/forest_pruned.bmp";
    ...
    for ( int i = 0 ; i < height ; ++i )
		...
		int p[3] = {1, 95, 0};	// image quality for saving
        if ( !cvSaveImage(fileNameOutput,output_img, p))

			printf("/!\\/!\\/!\\/!\\/!\\/!\\Could not save the resultant image. Check the path of saving.../!\\/!\\/!\\/!\\/!\\/!\\\n");

		// cvSaveImage("mask.bmp",maskimage,p);
...
// It will be useful so far to measure the quality of the recontructed areas against the original ones 
		...
		printf("The CPU time is %i minutes and %i secondes\n",time,secondes);
